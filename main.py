...
# API Keys loaded from environment variables (for Render deployment)
...
async def get_weather(location: str, units: str, openweather_api_key: str) -> str:
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:  # Increased timeout for production
        ...
        try:
            async with websockets.connect(
                ...
                async def receive_from_murf():
                    ...
                    try:
                        while True:
                            response = await asyncio.wait_for(murf_ws.recv(), timeout=30.0)  # Increased timeout
            ...
            logger.error(f"Error connecting to Murf: {e}")
